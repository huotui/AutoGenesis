...
class LLMClient:
    ...
    Supports Azure OpenAI and local LLM endpoints for web page evaluation.
    """
    ...
    def evaluate_task(self, task_info: str, image_data: bytes = None) -> ImgTaskResponse:
        """
        Evaluates whether a given screenshot satisfies the specified task description.

        Args:
            task_info (str): The task description to evaluate.
            image_data (bytes): The screenshot image data in binary format.
    ...
    def evaluate_task_with_azure(self, task_info: str, image_data: bytes = None) -> ImgTaskResponse:
        """
        Evaluates task using Azure OpenAI.
    ...
    def evaluate_task_with_local_lm(self, task_info: str, image_data: bytes = None) -> ImgTaskResponse:
        """
        Evaluates task using a local LLM endpoint.
    ...
    def evaluate_web_task(self, task_info: str, page_source: str = "", image_data: bytes = None) -> WebTaskResponse:
        """
        Evaluates whether a web page satisfies the specified task description.
        Uses both page source and screenshot for comprehensive evaluation.

        Args:
            task_info (str): The task description to evaluate.
            page_source (str): The HTML source of the web page.
            image_data (bytes): The screenshot image data in binary format (optional).

        Returns:
            WebTaskResponse: A structured response containing the evaluation result, reasoning, and found elements.
        """
        if self.api_key and self.azure_endpoint:
            return self.evaluate_web_task_with_azure(task_info, page_source, image_data)
        elif self.local_lm_endpoint:
            return self.evaluate_web_task_with_local_lm(task_info, page_source, image_data)
        raise Exception("No valid LLM provider configured.")

    def evaluate_web_task_with_azure(self, task_info: str, page_source: str = "", image_data: bytes = None) -> WebTaskResponse:
        """
        Evaluates web task using Azure OpenAI.
        ...
        Returns:
            WebTaskResponse: A structured response containing the evaluation result.
        """
        model = self.get_azure_model()
        prompt = web_task_prompt(task_info, page_source)
        ...
        structured_llm = model.with_structured_output(WebTaskResponse)
        res = structured_llm.invoke([message])
        return res

    def evaluate_web_task_with_local_lm(self, task_info: str, page_source: str = "", image_data: bytes = None) -> WebTaskResponse:
        """
        Evaluates web task using a local LLM endpoint.
        ...
        """
        prompt = web_task_prompt(task_info, page_source)
        output_format = WebTaskResponse.get_prompt_format()
        user_content = []
        ...
        full_response = full_response.replace("```json", "").replace("```", "")
        res = WebTaskResponse.model_validate_json(full_response)
        return res
